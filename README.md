...
**Защищённые (Bearer JWT; join тоже под auth — нужен стабильный user_id из анонимной сессии):**

| Метод | Путь | Статус | Описание |
|---|---|---|---|
| POST | `/api/v1/games` | ✅ | Создать игру (имя, 1–8 сторон, bbox, respawn_seconds) |
| POST | `/api/v1/games/join` | ✅ | Вход по коду стороны (возвращает game_id, side, role, respawn_seconds, map_pack_url) |
| PATCH | `/api/v1/games/:id` | ⏳ 501 | Изменить настройки игры (зарезервировано) |
| POST | `/api/v1/games/:id/map-pack` | ✅ | Загрузить ссылку на MBTiles в Storage + bbox (organizer) |
| POST | `/api/v1/games/:id/qr` | ➖ | QR рендерится на клиенте из join_code (заглушка) |
| GET | `/api/v1/games/:id/sides` | ✅ | Список сторон (любой член) |
| GET | `/api/v1/games/:id/squads` | ✅ | Список отрядов всех сторон |
| POST | `/api/v1/games/:id/squads` | ✅ | Создать отряд (organizer / side_commander) |
| GET | `/api/v1/games/:id/members` | ✅ | Список подключившихся (фильтр по правам) |
| PATCH | `/api/v1/games/:id/members/:uid` | ✅ | Назначить сторону / отряд / роль / позывной |
| POST | `/api/v1/games/:id/markers` | ✅ | Создать метку (visibility + bbox-валидация + WS-broadcast) |
| GET | `/api/v1/games/:id/markers` | ✅ | Видимые метки (фильтрация на сервере) |
| GET | `/api/v1/games/:id/spawn-points` | ✅ | Точки возрождения игры (любой член) |
| POST | `/api/v1/games/:id/spawn-points` | ✅ | Поставить точку возрождения (organizer) |
| POST | `/api/v1/games/:id/kills` | ✅ | Зафиксировать «убит» (опц. event_id) |
| POST | `/api/v1/games/:id/respawn` | ✅ | Зафиксировать возрождение (опц. event_id) |
| POST | `/api/v1/games/:id/events/sync` | ✅ | Батч офлайн-событий из outbox (idempotent) |
| GET | `/api/v1/ws?game=...` | ✅ | WebSocket (позиции + события, фильтрация на сервере) |
...
├── mobile/                              # Flutter
│   ├── lib/
│   │   ├── main.dart                    # init Supabase + TTS, ProviderContainer
│   │   ├── app.dart                     # GoRouter (splash/lobby/create/command/battle/dead)
│   │   ├── features/
│   │   │   ├── splash/                  # тёмно-зелёный сплэш
│   │   │   ├── lobby/                   # QR-сканер, ввод кода, prefetch map-pack
│   │   │   ├── game_create/             # Bbox, нарезка тайлов, стороны, слайдер респауна
│   │   │   ├── command/                 # Распределение по отрядам, роли (drag&drop)
│   │   │   ├── battle_map/              # Карта, метки, spawn-points, компас, WS, TTS
│   │   │   ├── kill_state/              # Экран "убит" → азимут до мертвяка + таймер
│   │   │   └── voice/                   # tts_service.dart (очередь приоритетов)
│   │   └── core/
│   │       ├── auth/                    # Supabase anon sessions
│   │       ├── api/                     # api_client, games_api, markers_api, events_api
│   │       ├── ws/                      # ws_service.dart (reconnect + heartbeat)
│   │       ├── gps/                     # kalman_filter, gps_provider, motion_service, geo
│   │       ├── map/                     # mbtiles_server, tile_downloader, map_pack_cache
│   │       ├── sync/                    # event_sync_service.dart (offline outbox flush)
│   │       ├── session/                 # game_session.dart (Riverpod-модель сессии)
│   │       ├── config/                  # supabase_config.dart (gitignored)
│   │       └── storage/                 # database.dart (Drift: games/markers/events)
│   └── pubspec.yaml
│
├── backend/                             # Go API
│   ├── Dockerfile
│   ├── cmd/
│   │   ├── api/main.go
│   │   └── migrate/main.go              # forward-only мигратор *.up.sql
│   ├── internal/
│   │   ├── handler/                     # game, side, squad, member, marker, spawn, event, ws
│   │   ├── service/                     # бизнес-логика (game/marker/event), правила видимости
│   │   ├── repository/                  # sqlx-репозитории
│   │   ├── model/                       # доменные модели
│   │   ├── middleware/                  # JWT (HS256; ES256/JWKS — TODO)
│   │   └── websocket/                   # hub + member-кэш + фильтрация broadcast
│   ├── migrations/
│   │   ├── 001_init.up.sql              # profiles, games, sides, spawn_points
│   │   ├── 002_members.up.sql           # squads, game_members, roles, status
│   │   ├── 003_markers_events.up.sql    # markers (visibility), events (outbox-sink)
│   │   ├── 004_profiles_trigger.up.sql  # auth.users → profiles
│   │   └── 005_respawn_seconds.up.sql   # games.respawn_seconds
│   └── go.mod
│
└── .gitignore
```
